    ...
    <Draggable draggableId={task.id} index={index} isDragDisabled={!canEdit}>
      {(provided, snapshot) => (
        <div
          ...
          onClick={() => onEdit(task)}
          className={`bg-white dark:bg-stone-800/80 rounded-xl border p-3.5 mb-2.5 cursor-pointer
            transition-all duration-200 group
            ${
              snapshot.isDragging
                ? "shadow-xl border-brand-300 dark:border-brand-600 ring-2 ring-brand-100 dark:ring-brand-800/50 scale-[1.02] rotate-[1deg] cursor-grabbing"
